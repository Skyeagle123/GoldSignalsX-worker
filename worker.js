...
async function rateLimit(ip, env){
  ...
  const cur = Number(await env.GSX_KV.get(key) || 0);
  if (cur >= limit) return false;
  await env.GSX_KV.put(key, String(cur + 1), { expirationTtl: 90 });
  return true;
}
...
async function handlePrice(env){
  ...
  const ts = j.ts ? Number(j.ts) : Date.now();
  const price = Number(j.price ?? j.close ?? j.last ?? 0);
...
async function handleBars(url, env){
  ...
  if (rows && rows.length) return json(rows);
...
async function listTicks(env, fromMs, toMs){
  ...
  for (const d of daySpan(fromMs, toMs)){
    let cursor;
    do {
      const r = await env.GSX_KV.list({ prefix:`ticks:${d}:`, limit:1000, cursor });
      for (const k of r.keys){
        const ts = Number(k.name.split(':').pop());
        if (ts>=fromMs && ts<=toMs){
          const v = await env.GSX_KV.get(k.name);
...
async function handleExportCSV(url, env){
  const tf=url.searchParams.get('tf')||'1m';
  const rows=await d1Bars(env, tf, 20000);
  ...
  return new Response(csv, {
    headers:{
      ...
      'content-disposition':`attachment; filename="XAUUSD_${tf}.csv"`
    }
  });
}
...
async function handleNotify(req, env){
  ...
  const j = await r.json();
  if (!j.ok) return json({ error:'tg failed', tg:j }, 502);
  return json({ ok:true });
}

async function handleDecision(req, env){
...
function cors(res, origin){
  ...
  h.set('access-control-headers', 'content-type');

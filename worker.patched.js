...
//
// المتغيرات المدعومة من البيئة (Env):
//   UPSTREAM_URL     → مسار gold-ticks (اختياري)
//   ALLOW_ORIGINS    → قائمة Origins للـ CORS (JSON أو *)
//   GSX_KV           → Cloudflare KV (اختياري)
//   GSX_DB           → D1 DB (اختياري)
//   TELEGRAM_TOKEN / TELEGRAM_CHAT  → للتنبيه
...
export default {
  async fetch(req, env, ctx) {
    ...
    // إزالة السلاشات الزائدة من اليمين بدون Regex لتفادي خطأ "Invalid regular expression"
    while (path.length > 1 && path.endsWith('/')) path = path.slice(0, -1);
    const method = req.method.toUpperCase();

    // ---------- CORS ----------
    const origin = req.headers.get('Origin') || '';
    const allow = parseAllow(env.ALLOW_ORIGINS); // e.g. ["https://skyeagle123.github.io","*"]
    const corsHeaders = makeCorsHeaders(origin, allow);
    if (method === 'OPTIONS') return new Response(null, { status: 204, headers: corsHeaders });

    try {
      // تهيئة آمنة اختيارية لقاعدة D1 (مرة واحدة كل تنفيذ)
      ...
      if (path === '/price') {
        ...
        // تخزين اختياري في KV
        if (env.GSX_KV && data && Number.isFinite(data.price)) {
...
if (path === '/import.csv' && method === 'POST') {
  ...
  return json({ ok:true, imported: rows.length, tf });
}
if (path === '/bars') {
...
async function getPriceUnified(env) {
  // 1) gold-ticks (UPSTREAM_URL) إن وُجد
  ...
  try {
    ...
    await env.GSX_DB.exec(`
      CREATE TABLE IF NOT EXISTS bars (
        ...
        tf INTEGER NOT NULL,   -- timeframe minutes
        o REAL NOT NULL,
        h REAL NOT NULL,
        l REAL NOT NULL,
        c REAL NOT NULL,
    ...
    // تجاهل أي خطأ في التهيئة (لا نكسر الطلب)
  }
}